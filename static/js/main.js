...
function initAdminPage() {
    ...
    // 【主要修改處】為所有區域列表初始化 SortableJS
    Object.values(areaLists).forEach(listEl => {
        new Sortable(listEl, {
            group: 'doctors', // 允許在列表間拖曳
            animation: 150,
            handle: '.drag-handle',
        });
    });
    ...
    async function loadDoctorSettings() {
        ...
        try {
            ...
            if (doctorsToRender.length > 0) {
                doctorsToRender
                    .sort(([, a], [, b]) => (a.points_limit > b.points_limit) ? -1 : 1) // 預先按點數排序
                    .forEach(([name, docData]) => renderDoctorItem({ name, data: docData }));
            } else {
                 areaLists.A.innerHTML = `<li class="list-group-item text-muted">本月尚無資料</li>`;
            }
    ...
    function loadTemplateData() { 
        Object.values(areaLists).forEach(list => list.innerHTML = '');
        Object.entries(doctorTemplate)
            .sort(([, a], [, b]) => (a.points_limit > b.points_limit) ? -1 : 1)
            .forEach(([name, template]) => {
                const rowData = { name: name, data: { ...template, submitted: true } }; 
                renderDoctorItem(rowData); 
            }); 
    ...
    function renderDoctorItem({ name, data }) {
        ...
        // 為了儲存，把預休和提交狀態也放進 data attribute
        item.dataset.daysOff = JSON.stringify(data.days_off || []);
        item.dataset.submitted = data.submitted || false;

        const daysOffText = data.days_off && data.days_off.length > 0 ? data.days_off.join(', ') : '無預休';
        const submittedIcon = data.submitted 
            ? `<i class="bi bi-check-circle-fill" title="已提交"></i>` 
            : `<i class="bi bi-x-circle-fill" title="未提交"></i>`;
        ...
        item.innerHTML = `
            ...
            <div class="doctor-list-item-content">
                <div>
                    ...
                    <div class="days-off-info">${submittedIcon} ${daysOffText}</div>
                </div>
